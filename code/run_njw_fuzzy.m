function [U1, U2, U3, U4] = run_njw_fuzzy(data, nGroups, labels, k1, k2, k3, k4, n4)


% Runs the NJW algorithm using different similarities approaches using
% fuzzy c-means instead of k-means
    ...
    % Output: 
        % U1 - the clustering obtained using the Gaussian Kernel
        % U2 - the clustering obtained using the k-NN
        % U3 - the clustering obtained using the mutual k-NN
        % U4 - the clustering obtained using the fuzzy similarity
    ...
    [C1, U1] = gcut_fuzzy(A1, nGroups);
    [C2, U2] = gcut_fuzzy(A2, nGroups);
    [C3, U3] = gcut_fuzzy(A3, nGroups);
    [C4, U4] = gcut_fuzzy(A4, nGroups);
